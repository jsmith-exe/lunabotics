...
// CAN frames so linear-actuator analog feedback is still observed and the
// serial-to-CAN adapter RX buffer does not back up.
...
public:
    ...
    const hardware_interface::HardwareInfo & info) override
  {
    ...
    RCLCPP_WARN(logger_, "Linear actuator analogue-voltage feedback is sniffed from raw CAN status 3 frames");
    ...
    const rclcpp_lifecycle::State &) override
  {
    ...
    RCLCPP_WARN(logger_, "Sending guaranteed zero-duty stop burst to all 4 motors and both linear actuators");
    ...
    const rclcpp::Duration &) override
  {
    ...
    // Global heartbeat, independent of individual motor writes, so the linear
    // actuators (and idle wheels) stay alive even when no wheel command is active.
    maybe_send_heartbeat();

    // Closed-loop actuator position servos are intentionally independent of the
    // wheels: run them every write() cycle before the wheel command gating below.
...
private:
  ...
  // so a held command parks the actuator instead of stalling at an end-stop.
